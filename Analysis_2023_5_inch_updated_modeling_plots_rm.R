# This script investigated if the plots removed from the main area of Michaux State Forest would influence the response curve
# DBH threshold is still > 5 inches
# The presence/absence patterns based on the threhold between 5 inches and 7.8 inches were also investigated
...
# Load the Plot ID for CFI data only
CFI_plot_ID <- read.csv("~/Desktop/charcoal_burning/Modeling/CFI_plot_ID.csv", sep="")
...
glm_chestnutoak_occr <- glm(chestnutoak_occr ~ EST*DIST + EST*COUNT + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                            ...
                            data = chestnutoak[chestnutoak$PlotID %in% CFI_plot_ID$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_chestnutoak_occr, direction = "both")

glm_chestnutoak_occr_new <- glm(chestnutoak_occr ~ EST + DIST + COUNT + FDAY + 
                                  DEPTH + SIEVE200 + ERR15 + ROUGH + TPI150 + EST:DIST + EST:COUNT,
                                data = chestnutoak[chestnutoak$PlotID %in% CFI_plot_ID$PlotID, -c(1:9)],
...
chestnutoak_data_rf <- chestnutoak[, -c(1:9)]
chestnutoak_data_rf$chestnutoak_occr <- as.factor(chestnutoak_data_rf$chestnutoak_occr)
table(chestnutoak_data_rf$chestnutoak_occr[chestnutoak_data_rf$DIST<=15])
table(chestnutoak_data_rf[!chestnutoak$PlotID %in% plot_far$PlotID, ]$chestnutoak_occr)

# With legacy variables
set.seed(1)
chestnutoak_rf_new_w <- randomForest(#chestnutoak_occr ~ DIST + COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                       #SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                     x = chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, -1], 
                                     y = chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, 1],
                                     #data = chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, ],
                                     data = chestnutoak_data_rf[!chestnutoak$PlotID %in% plot_far$PlotID, ],
                                     importance = TRUE,
                                     sampsize = c(16, 16))
print(chestnutoak_rf_new_w)
varImpPlot(chestnutoak_rf_new_w)
partialPlot(chestnutoak_rf_new_w, chestnutoak_data_rf, DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
chestnutoak_rf_cv_w <- rf.crossValidation(chestnutoak_rf_new_w, 
                                          #chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, -1], 
                                          xdata = chestnutoak_data_rf[!chestnutoak$PlotID %in% plot_far$PlotID, -1],
                                          ydata = chestnutoak_data_rf[!chestnutoak$PlotID %in% plot_far$PlotID, 1],
                                          p = 0.2, n = 100, ntree=500,
                                          sampsize = c(15, 15))
summary(chestnutoak_rf_cv_w$cross.validation$cv.oob)

par(mfrow=c(1,2)) 
plot(rf.cv, type = "cv", main = "CV producers accuracy")
plot(rf.cv, type = "model", main = "Model producers accuracy")

# Without legacy variables
set.seed(2)
chestnutoak_rf_new_wo <- randomForest(chestnutoak_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                        SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                      #data = chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, ], 
                                      data = chestnutoak_data_rf[!chestnutoak$PlotID %in% plot_far$PlotID, ],
                                      importance = TRUE,
                                      sampsize = c(16, 16))
print(chestnutoak_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
chestnutoak_rf_cv_wo <- rf.crossValidation(chestnutoak_rf_new_wo, chestnutoak_data_rf[chestnutoak_data_rf$DIST<=15, -c(1, 18:20)], 
                                           p = 0.2, n = 100, ntree=500,
                                           sampsize = c(28, 28))
summary(chestnutoak_rf_cv_wo$cross.validation$cv.oob)
...
# Northern red oak
redoak_data_rf <- redoak[, -c(1:9)]
redoak_data_rf$redoak_occr <- as.factor(redoak_data_rf$redoak_occr)
table(redoak_data_rf$redoak_occr[redoak_data_rf$DIST<=15])

# With legacy variables
set.seed(1)
redoak_rf_new_w <- randomForest(x = redoak_data_rf[redoak_data_rf$DIST<=15, -1],
                                y = redoak_data_rf[redoak_data_rf$DIST<=15, 1],
                                data = redoak_data_rf[redoak_data_rf$DIST<=15, ], importance = TRUE,
                                sampsize = c(40, 40))
print(redoak_rf_new_w)
varImpPlot(redoak_rf_new_w)
partialPlot(redoak_rf_new_w, redoak_data_rf, DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
redoak_rf_cv_w <- rf.crossValidation(redoak_rf_new_w, redoak_data_rf[redoak_data_rf$DIST<=15, -1], 
                                     p = 0.2, n = 100, ntree=500,
                                     sampsize = c(32, 32))
summary(redoak_rf_cv_w$cross.validation$cv.oob)

# Without legacy variables
set.seed(2)
redoak_rf_new_wo <- randomForest(redoak_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                   SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                 data = redoak_data_rf[redoak_data_rf$DIST<=15, ], importance = TRUE,
                                 sampsize = c(40, 40))
print(redoak_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
redoak_rf_cv_wo <- rf.crossValidation(redoak_rf_new_wo, redoak_data_rf[redoak_data_rf$DIST<=20, -c(1, 18:20)], 
                                      p = 0.2, n = 100, ntree=500,
                                      sampsize = c(32, 32))
summary(redoak_rf_cv_wo$cross.validation$cv.oob)

# Scarlet oak
scarletoak_data_rf <- scarletoak[, -c(1:9)]
scarletoak_data_rf$scarletoak_occr <- as.factor(scarletoak_data_rf$scarletoak_occr)
table(scarletoak_data_rf$scarletoak_occr[scarletoak_data_rf$DIST<=15])

# With legacy variables
set.seed(1)
scarletoak_rf_new_w <- randomForest(x = scarletoak_data_rf[scarletoak_data_rf$DIST<=15, -1],
                                    y = scarletoak_data_rf[scarletoak_data_rf$DIST<=15, 1],
                                    data = scarletoak_data_rf[scarletoak_data_rf$DIST<=15, ], importance = TRUE,
                                    sampsize = c(57, 57)
                                    )
print(scarletoak_rf_new_w)
varImpPlot(scarletoak_rf_new_w)
partialPlot(scarletoak_rf_new_w, scarletoak_data_rf, DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
scarletoak_rf_cv_w <- rf.crossValidation(scarletoak_rf_new_w, scarletoak_data_rf[scarletoak_data_rf$DIST<=15, -1], 
                                         p = 0.2, n = 100, ntree=500,
                                         sampsize = c(45, 45)
                                         )
summary(scarletoak_rf_cv_w$cross.validation$cv.oob)

# Without legacy variables
set.seed(2)
scarletoak_rf_new_wo <- randomForest(scarletoak_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                       SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                     data = scarletoak_data_rf[scarletoak_data_rf$DIST<=15, ], importance = TRUE,
                                     sampsize = c(57, 57)
                                     )
print(scarletoak_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
scarletoak_rf_cv_wo <- rf.crossValidation(scarletoak_rf_new_wo, scarletoak_data_rf[scarletoak_data_rf$DIST<=15, -c(1, 18:20)], 
                                          p = 0.2, n = 100, ntree=500,
                                          sampsize = c(45, 45)
                                          )
summary(scarletoak_rf_cv_wo$cross.validation$cv.oob)

# White oak
whiteoak_data_rf <- whiteoak[, -c(1:9)]
whiteoak_data_rf$whiteoak_occr <- as.factor(whiteoak_data_rf$whiteoak_occr)
table(whiteoak_data_rf$whiteoak_occr[whiteoak_data_rf$DIST<=15])

# With legacy variables
set.seed(1)
whiteoak_rf_new_w <- randomForest(x = whiteoak_data_rf[whiteoak_data_rf$DIST<=15, -1],
                                  y = whiteoak_data_rf[whiteoak_data_rf$DIST<=15, 1],
                                  data = whiteoak_data_rf[whiteoak_data_rf$DIST<=15, ], importance = TRUE,
                                  sampsize = c(36, 36))
print(whiteoak_rf_new_w)
varImpPlot(whiteoak_rf_new_w)
partialPlot(whiteoak_rf_new_w, whiteoak_data_rf, DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
whiteoak_rf_cv_w <- rf.crossValidation(whiteoak_rf_new_w, whiteoak_data_rf[whiteoak_data_rf$DIST<=15, -1], 
                                       p = 0.2, n = 100, ntree=500,
                                       sampsize = c(28, 28))
summary(whiteoak_rf_cv_w$cross.validation$cv.oob)

# Without legacy variables
set.seed(2)
whiteoak_rf_new_wo <- randomForest(whiteoak_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                     SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                   data = whiteoak_data_rf[whiteoak_data_rf$DIST<=15, ], importance = TRUE,
                                   sampsize = c(36, 36))
print(whiteoak_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
whiteoak_rf_cv_wo <- rf.crossValidation(whiteoak_rf_new_wo, whiteoak_data_rf[whiteoak_data_rf$DIST<=15, -c(1, 18:20)], 
                                        p = 0.2, n = 100, ntree=500,
                                        sampsize = c(28, 28))
summary(whiteoak_rf_cv_wo$cross.validation$cv.oob)

# Pitch pine
pitchpine_data_rf <- pitchpine[, -c(1:9)]
pitchpine_data_rf$pitchpine_occr <- as.factor(pitchpine_data_rf$pitchpine_occr)
table(pitchpine_data_rf$pitchpine_occr[pitchpine_data_rf$DIST<=15])

# With legacy variables
set.seed(1)
pitchpine_rf_new_w <- randomForest(x = pitchpine_data_rf[pitchpine_data_rf$DIST<=15, -1],
                                   y = pitchpine_data_rf[pitchpine_data_rf$DIST<=15, 1],
                                   data = pitchpine_data_rf[pitchpine_data_rf$DIST<=15, ], importance = TRUE,
                                   sampsize = c(45, 45))
print(pitchpine_rf_new_w)
varImpPlot(pitchpine_rf_new_w)
partialPlot(pitchpine_rf_new_w, pitchpine_data_rf, DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
pitchpine_rf_cv_w <- rf.crossValidation(pitchpine_rf_new_w, pitchpine_data_rf[pitchpine_data_rf$DIST<=15, -1], 
                                        p = 0.2, n = 100, ntree=500,
                                        sampsize = c(36, 36))
summary(pitchpine_rf_cv_w$cross.validation$cv.oob)

# Without legacy variables
set.seed(2)
pitchpine_rf_new_wo <- randomForest(pitchpine_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                      SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                    data = pitchpine_data_rf[pitchpine_data_rf$DIST<=15, ], importance = TRUE,
                                    sampsize = c(45, 45))
print(pitchpine_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
pitchpine_rf_cv_wo <- rf.crossValidation(pitchpine_rf_new_wo, pitchpine_data_rf[pitchpine_data_rf$DIST<=15, -c(1, 18:20)], 
                                         p = 0.2, n = 100, ntree=500,
                                         sampsize = c(36, 36))
summary(pitchpine_rf_cv_wo$cross.validation$cv.oob)

# Red maple
redmaple_data_rf <- redmaple[, -c(1:9)]
redmaple_data_rf$redmaple_occr <- as.factor(redmaple_data_rf$redmaple_occr)
table(redmaple_data_rf$redmaple_occr[redmaple_data_rf$DIST<=15])

# With legacy variables
set.seed(1)
redmaple_rf_new_w <- randomForest(x = redmaple_data_rf[redmaple_data_rf$DIST<=15, -1],
                                  y = redmaple_data_rf[redmaple_data_rf$DIST<=15, 1],
                                  data = redmaple_data_rf[redmaple_data_rf$DIST<=15, ], importance = TRUE,
                                  sampsize = c(53, 53))
print(redmaple_rf_new_w)
varImpPlot(redmaple_rf_new_w)
partialPlot(redmaple_rf_new_w, redmaple_data_rf, DIST, "1")
partialPlot(redmaple_rf_new_w, redmaple_data_rf[redmaple_data_rf$EST > 123&redmaple_data_rf$EST < 160, ], DIST, "1")

# Cross-validation
# For the whole data set
set.seed(1)
redmaple_rf_cv_w <- rf.crossValidation(redmaple_rf_new_w, redmaple_data_rf[redmaple_data_rf$DIST<=15, -1], 
                                       p = 0.2, n = 100, ntree=500,
                                       sampsize = c(42, 42))
summary(redmaple_rf_cv_w$cross.validation$cv.oob)

# Without legacy variables
set.seed(2)
redmaple_rf_new_wo <- randomForest(redmaple_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                     SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                   data = redmaple_data_rf[redmaple_data_rf$DIST<=15, ], importance = TRUE,
                                   sampsize = c(53, 53))
print(redmaple_rf_new_wo)

# Cross-validation
# For the whole data set
set.seed(2)
redmaple_rf_cv_wo <- rf.crossValidation(redmaple_rf_new_wo, redmaple_data_rf[redmaple_data_rf$DIST<=15, -c(1, 18:20)], 
                                        p = 0.2, n = 100, ntree=500,
                                        sampsize = c(42, 42))
summary(redmaple_rf_cv_wo$cross.validation$cv.oob)



# Save the RData in case
save.image("~/Desktop/charcoal_burning/Modeling/Analysis_2023_5_inch_updated_modeling_plots_rm_RF.RData")
...
# Distance to hearth
visreg(glm_chestnutoak_occr_new, "DIST", data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)],
       scale = "response", gg = TRUE, line = list(size = .8),
       rug = 0,
       overlay = TRUE,  band = FALSE, nn = 200,
       print.cond = TRUE, type = "conditional") +
...
# Count
visreg(glm_chestnutoak_occr_new, "COUNT", data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)],
       scale = "response", gg = TRUE, line = list(size = .8),
       rug = 0,
       overlay = TRUE,  band = FALSE, nn = 200,
       print.cond = TRUE, type = "conditional") +
  scale_x_continuous(name = "Number of charcoal hearths surrounding forested plot") +
  scale_y_continuous(name = "Probability of presence", breaks = c(0, 0.5, 1)) +
  scale_color_manual( values = c("#FF5733", "#FFC300", "#154360")) +
  labs(color = "Mean stand <br>age (year)")+
  theme_classic() +
...
glm_chestnutoak_occr_aic <- glm(chestnutoak_occr ~ MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                                  SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP,
                                data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_chestnutoak_occr_aic, direction = "both")


glm_chestnutoak_occr_new_wo <- glm(chestnutoak_occr ~ MMAX + FDAY + DEPTH + SIEVE200 + 
                                     ERR15 + SOILSP + TPI150 + TPI2000,
                                   data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                   family = binomial(link = "logit"))
...
# Chestnut oak
glm_chestnutoak_occr_new_leg <- glm(chestnutoak_occr ~ EST + DIST + COUNT + EST:DIST + EST:COUNT,
                                    data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                    family = binomial(link = "logit"))

glm_chestnutoak_occr_new_nonleg <- glm(chestnutoak_occr ~ MMAX + 
                                         DEPTH + SIEVE200 + ERR15 + SOILSP + TPI150,
                                       data = chestnutoak[!chestnutoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                       family = binomial(link = "logit"))
...
glm_redoak_occr <- glm(redoak_occr ~ DIST*EST + COUNT*EST + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                         SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP + EST + COUNT + DIST,
                       data = redoak[!redoak$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_redoak_occr, direction = "both")


glm_redoak_occr_new <- glm(redoak_occr ~ EST + COUNT + PRATIO + CLAY + IMI + 
                             SOILSP + BULK + EST:COUNT,
                           data = redoak[!redoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                           family = binomial(link = "logit"))
...
glm_redoak_occr_new_wo <- glm(redoak_occr ~ BULK + CLAY + IMI + ROUGH + SOILSP,
                              data = redoak[!redoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                              family = binomial(link = "logit"))
...
# Northern red oak
glm_redoak_occr_new_leg <- glm(redoak_occr ~ EST + COUNT + EST:COUNT,
                               data = redoak[!redoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                               family = binomial(link = "logit"))

glm_redoak_occr_new_nonleg <- glm(redoak_occr ~ BULK + CLAY + IMI + ROUGH + SOILSP,
                                  data = redoak[!redoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                  family = binomial(link = "logit"))
...
glm_redmaple_occr <- glm(redmaple_occr ~ DIST*EST + COUNT*EST + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                           SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP + EST + COUNT + DIST,
                         data = redmaple[!redmaple$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_redmaple_occr, direction = "both")


glm_redmaple_occr_new <- glm(redmaple_occr ~ DIST + EST + IMI + TPI150 + TPI2000 + 
                               DIST:EST,
                             data = redmaple[!redmaple$PlotID %in% plot_far$PlotID, -c(1:9)],
                             family = binomial(link = "logit"))
...
glm_redmaple_occr_new_wo <- glm(redmaple_occr ~ FDAY + TPI150 + TPI2000,
                                data = redmaple[!redmaple$PlotID %in% plot_far$PlotID, -c(1:9)],
                                family = binomial(link = "logit"))
...
glm_scarletoak_occr <- glm(scarletoak_occr ~ DIST*EST + COUNT*EST + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                             SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP + EST + COUNT + DIST,
                           data = scarletoak[!scarletoak$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_scarletoak_occr, direction = "both")


glm_scarletoak_occr_new <- glm(scarletoak_occr ~ DIST + EST + COUNT + MMAX + PRATIO + 
                                 DEPTH + IMI + TPI150 + DIST:COUNT,
                              data = scarletoak[!scarletoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                              family = binomial(link = "logit"))
...
glm_scarletoak_occr_new_wo <- glm(scarletoak_occr ~ PRATIO + BULK + DEPTH + CLAY + 
                                    PERM + IMI + TPI150 + FDAY,
                                  data = scarletoak[!scarletoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                  family = binomial(link = "logit"))
...
glm_pitchpine_occr <- glm(pitchpine_occr ~ DIST*EST + COUNT*EST + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                            SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP + EST + COUNT + DIST,
                          data = pitchpine[!pitchpine$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_pitchpine_occr, direction = "both")

# DIST was not significant but we still let it in the model
glm_pitchpine_occr_new <- glm(pitchpine_occr ~ EST + COUNT + PRATIO + SIEVE200 + 
                                TRASP + EST:COUNT,
                              data = pitchpine[!pitchpine$PlotID %in% plot_far$PlotID, -c(1:9)],
                              family = binomial(link = "logit"))
...
glm_pitchpine_occr_new_wo <- glm(pitchpine_occr ~ PRATIO + CLAY + PERM + SIEVE200 + 
                                   TRASP,
                                 data = pitchpine[!pitchpine$PlotID %in% plot_far$PlotID, -c(1:9)],
                                 family = binomial(link = "logit"))
...
glm_whiteoak_occr <- glm(whiteoak_occr ~ DIST*EST + COUNT*EST + DIST*COUNT + MMAX + FDAY + PRATIO + BULK + DEPTH + OM + CLAY + PERM +
                           SIEVE200 + ERR15 + IMI + ROUGH + SOILSP + TPI150 + TPI2000 + TRASP + EST + COUNT + DIST,
                         data = whiteoak[!whiteoak$PlotID %in% plot_far$PlotID, -c(1:9)], family = binomial(link = "logit"))
stepAIC(glm_whiteoak_occr, direction = "both")


glm_whiteoak_occr_new <- glm(whiteoak_occr ~ DIST + EST + COUNT + MMAX + FDAY + 
                               PRATIO + DEPTH + OM + CLAY + PERM + SIEVE200 + IMI + ROUGH + 
                               SOILSP + TPI150 + TPI2000 + TRASP,
                             data = whiteoak[!whiteoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                             family = binomial(link = "logit"))
...
glm_whiteoak_occr_new_wo <- glm(whiteoak_occr ~ MMAX + FDAY + DEPTH + CLAY + PERM + 
                                  SIEVE200 + IMI + TPI2000 + TRASP,
                                data = whiteoak[!whiteoak$PlotID %in% plot_far$PlotID, -c(1:9)],
                                family = binomial(link = "logit"))
